"""Add provider-neutral external identities and provider-only users.

Revision ID: 0034_external_identities
Revises: 0033_snapshot_provenance
"""
...
revision = "0034_external_identities"
down_revision = "0033_snapshot_provenance"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    ...
    if null_password_users:
        raise RuntimeError(
            "Refusing AUTH-2A downgrade: provider-only users have no password credential."
        )
