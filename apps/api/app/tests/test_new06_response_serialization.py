...
from app.services.profile_projection import CREDENTIAL_KEY_NAMES

PLAINTEXT_CANARY = "NEW06-PLAINTEXT-CANARY"
KEY_CANARY = "NEW06-ENCRYPTION-KEY-CANARY"
...
def test_repaired_document_route_preserves_new07_provider_and_snapshot_security(
    client: TestClient, provider_calls: list[dict],
) -> None:
    headers = signup(client, "new07@mailbox.test-domain.co")
    complete_profile(client, headers, "NEW07 Safe User")
    stored = client.put(
        "/profile/workday-credentials",
        headers=headers,
        json={"username": "new07@example.com", "password": PLAINTEXT_CANARY},
    )
    ...
    try:
        ...
        ciphertext = profile.workday_password_ciphertext
        assert ciphertext
    finally:
        db.close()

    job_id = seed_job("Secure", "n6-new07")
    response = client.post(
        f"/jobs/{job_id}/documents/cover_letter", headers=headers, json={}
    )
    assert response.status_code == 200, response.text
    assert provider_calls, "no provider request was made"

    outgoing = json.dumps(provider_calls, default=str)
    for canary in (ciphertext, PLAINTEXT_CANARY, KEY_CANARY):
        assert canary not in outgoing
    for key in CREDENTIAL_KEY_NAMES:
        assert f'"{key}"' not in outgoing
    assert "NEW07 Safe User" in outgoing

    snapshot_response = client.post(
        f"/jobs/{job_id}/generate-resume", headers=headers, json={}
    )
    assert snapshot_response.status_code == 200, snapshot_response.text
    document_id = snapshot_response.json()["document_id"]
    db = db_session()
    try:
        document = db.get(E.GeneratedDocument, document_id)
        snapshot = document.source_profile_snapshot
    finally:
        db.close()
    serialized = json.dumps(snapshot, default=str)
    for canary in (ciphertext, PLAINTEXT_CANARY, KEY_CANARY):
        assert canary not in serialized
    assert "workday_password_ciphertext" not in serialized
    assert snapshot["profile"]["full_name"] == "NEW07 Safe User"
