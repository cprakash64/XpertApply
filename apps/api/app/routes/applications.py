...
def _confirmation_tracker(db: Session, session: ApplicationSession) -> ApplicationTracker:
    """Return the session owner's provisional tracker without claiming submission."""
    tracker = db.scalar(
...
def require_confirmation(
    ...
    response = {"ok": True, "confirmation_required_at": tracker.confirmation_required_at}
    db.commit()
    return response
...
def _serialize_session(db: Session, session: ApplicationSession) -> dict[str, Any]:
    ...
    return {
        ...
        "confirmation_prompt_dismissed_at": (
            tracker.confirmation_prompt_dismissed_at if tracker else None
        ),
    }
