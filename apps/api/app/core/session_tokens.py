...
# Same single-element allow-list as app.core.security: the accepted algorithm is
# stated by this service, never read from the token header.
ALGORITHM = "HS256"
...
def _decode(token: str) -> dict[str, Any] | None:
    """Claims of a valid scoped token, or None.

    `InvalidTokenError` is PyJWT's base for bad signature, expiry, malformed
    input and a disallowed algorithm alike, so every rejection stays a None —
    the same boundary `JWTError` provided, and never a 500.
    """
    try:
