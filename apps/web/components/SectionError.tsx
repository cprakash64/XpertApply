...
export function SectionError({
  ...
  return (
    <div
      role="alert"
      className="rounded-card border border-status-danger bg-surface-subtle p-5"
      data-testid="section-error"
    >
      <div className="flex items-start gap-3">
        <AlertTriangle aria-hidden="true" className="mt-0.5 h-5 w-5 shrink-0 text-status-danger" />
        <div className="min-w-0 flex-1">
          <h3 className="text-base font-semibold text-foreground">{title}</h3>
          <p className="mt-1 text-sm text-foreground-secondary">{error.message}</p>

          <div className="mt-4 flex flex-wrap gap-3">
            {/* Recovery, not promotion. This card is an error state that can
              * appear several times on one screen, and a navy primary here would
              * out-rank the page's real call to action and make a failure look
              * like the thing to do next. Retry stays the emphasised choice
              * (secondary), the way back stays quieter (ghost). */}
