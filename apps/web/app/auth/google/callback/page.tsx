...
export default function GoogleCallbackPage() {
  ...
  return <main className="mx-auto flex min-h-screen max-w-lg items-center px-5 py-12">
    <section className="w-full rounded-[28px] border border-line bg-panel p-8" aria-labelledby="google-auth-title">
      ...
      {error && <p id="google-link-error" ref={errorRef} tabIndex={-1} className="mt-4 break-words rounded-xl border border-[var(--danger-border)] bg-[var(--danger-surface)] p-3 text-sm text-[var(--danger)]" role="alert">{error}</p>}
      {linkTicket && <form onSubmit={link} noValidate className="mt-5">
        ...
        <input ref={passwordRef} id="google-link-password" type="password" autoComplete="current-password" required value={password} aria-invalid={error ? "true" : undefined} aria-describedby={error ? "google-link-error" : undefined} onChange={(event) => { setPassword(event.target.value); if (error) setError(""); }} className="auth-input mt-2 h-12 w-full rounded-xl border border-line px-4" />
