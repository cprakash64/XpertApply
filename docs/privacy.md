...
XpertApply stores normal career profile data separately from sensitive demographic data. Sensitive demographic fields are optional, default to "Prefer not to answer", and are not used for job-fit scoring, ranking, resume generation, or cover-letter generation.
